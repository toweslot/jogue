...
function renderPainel(el) {
      ...
      <div class="pnl-modal">
        ...
        <div id="dep-step1">
          ...
          <div class="dep-amount-grid" id="dep-quick-row" style="margin-bottom:14px">
            ...
            <button class="pnl-quick" data-dep="150">R$150</button>
            <button class="pnl-quick" data-dep="250">R$250</button>
          </div>
  ...
  async function loadGameConfigs() {
    try {
      ...
      multIncGlobal    = parseFloat(cfg.multiplicador_incremento) >= 0.5 ? parseFloat(cfg.multiplicador_incremento) : 0.5;
  ...
  function _sugestaoPresetDeposito(valor) {
    ...
    if (v === 150) return { label: 'Mais chances', css: 'lbl-bonus' };
    if (v === 250) return { label: 'VIP', css: 'lbl-bonus' };
    return null;
  }
  function _gerarBotoesBonus(b) {
    ...
    const valoresFixos = [20, 30, 50, 100, 150, 250].filter(v => v >= depMin);
  ...
  // Permite abrir o modal de deposito a partir de outras telas (ex.: fim do demo)
  window.openDepositModal = openDepositModal;
  if (sessionStorage.getItem('abrir_deposito') === '1') {
    sessionStorage.removeItem('abrir_deposito');
    setTimeout(() => { try { openDepositModal(); } catch (_) {} }, 300);
  }
