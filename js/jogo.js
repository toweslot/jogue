...
function renderJogo(container) {
  ...
  let _infInicial = 0;
  let _kcoInicial = '';
  let _miInicial = 1, _mcInicial = 0.5, _mxInicial = 0;
  try {
    ...
    if (_p && parseFloat(_p.multiplicador_incremento) >= 0.5) _mcInicial = parseFloat(_p.multiplicador_incremento);
  ...
  const MULT_INC = parseFloat(partida.multiplicador_incremento) >= 0.5 ? parseFloat(partida.multiplicador_incremento) : 0.5;
  ...
  // 1o acerto = base (1x), 2o acerto = dobro (2x), depois +passo (2,5x / 3x / ...)
  function multPara(n) {
    let m = n <= 1 ? MULT_INI : (MULT_INI * 2 + (n - 2) * MULT_INC);
  ...
  function irParaDeposito() {
    ...
    sessionStorage.setItem('abrir_deposito', '1');
    if ((window.location.hash || '').split('?')[0] === '#painel') {
      if (typeof window.openDepositModal === 'function') {
        sessionStorage.removeItem('abrir_deposito');
  ...
  window.irParaDeposito = irParaDeposito;
  ...
  function mostrarModalDemo(valorGanho, resgatou) {
    ...
    // Usuario logado (jogando o demo dentro da plataforma) vai direto para o deposito.
    const logadoDemo = (() => { try { return !!API.getToken(); } catch (_) { return false; } })();
    const ctaTextoDemo = logadoDemo ? 'DEPOSITAR AGORA' : 'CRIAR CONTA GRATIS';
    const ctaAcaoDemo  = logadoDemo ? 'irParaDeposito()' : "navigate('#cadastro')";
    const ctaIconeDemo = logadoDemo
      ? '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" width="18" height="18"><rect x="2" y="6" width="20" height="13" rx="2"/><path d="M2 10h20"/><path d="M7 15h3"/></svg>'
      : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" width="18" height="18"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="19" y2="14"/><line x1="22" y1="11" x2="16" y2="11"/></svg>';
    ...
    overlay.innerHTML = `
      ...
      ">
        ...
        <div style="padding:0 20px 24px;display:flex;flex-direction:column;gap:10px">
          <button onclick="${ctaAcaoDemo}" style="
