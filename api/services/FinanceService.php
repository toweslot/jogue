...
final class FinanceService
{
    ...
    public function criarDeposito(array $user, array $payload): array
    {
        ...
        if ($emailIn === '' || !filter_var($emailIn, FILTER_VALIDATE_EMAIL)) {
            ...
            $emailIn = $slugNome . '.' . substr(bin2hex(random_bytes(4)), 0, 8) . '@towerslot.site';
        }
        ...
        try {
            ...
            if ($refId > 0) {
                ...
                if ($qtdDepositosAprovados === 1) {
                    ...
                    // Percentual padrao de todas as contas: 3% sobre qualquer valor.
                    // Se o admin definir percentual personalizado no painel, ele prevalece.
                    $indCfg = $this->configService->get('indicacao', []);
                    $perc = (float)($indCfg['comissao_nivel1_perc'] ?? 3);
