...
final class IndicacaoService
{
    ...
    public function info(array $user): array
    {
        ...
        // Compatível com SQLite e MySQL (sem funções JSON do MySQL).
        $stmtMeta = $this->pdo->prepare("SELECT meta_json FROM transacoes
                                     WHERE user_id=:uid AND tipo='bonus_indicacao' AND status='aprovado'");
        $stmtMeta->execute([':uid' => $uid]);
        $idsComissionados = [];
        foreach (($stmtMeta->fetchAll() ?: []) as $rowMeta) {
            $meta = json_decode((string)($rowMeta['meta_json'] ?? '{}'), true) ?: [];
            $idInd = (int)($meta['user_id_indicado'] ?? 0);
            if ($idInd > 0) {
                $idsComissionados[$idInd] = true;
            }
        }
        $totalComDeposito = count($idsComissionados);
        ...
        $comissaoPadrao = (float)($cfg['comissao_nivel1_perc'] ?? 3);
        $comissaoCustom = null;
        ...
        return [
            'link' => $this->baseUrl() . '/#landing?ref=' . urlencode((string)$user['codigo_indicacao']),
            ...
            'comissao_nivel1_perc' => (float)($cfg['comissao_nivel1_perc'] ?? 3),
    ...
    {
        $url = rtrim((string)($this->config['app']['url'] ?? ''), '/');
        $host = (string)($_SERVER['HTTP_HOST'] ?? '');
        if ($host !== '') {
            $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
            $current = $scheme . '://' . $host;
            // Sempre usar o dominio pelo qual o usuario acessou a plataforma.
