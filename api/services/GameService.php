...
final class GameService
{
    ...
    {
        $v = (float)($cfg['multiplicador_incremento'] ?? 0.5);
        // passo minimo de 0.5x apos o 2o acerto
        return $v >= 0.5 ? $v : 0.5;
    }
     ...
     * 1o acerto = base (1x), 2o acerto = dobro (2x), depois soma o passo (2.5x, 3x, ...).
     */
    private function multPara(int $acertos, array $cfg): float
    {
        $base = $this->multInicial($cfg);
        if ($acertos <= 1) {
            $m = $base;
        } else {
            $m = ($base * 2) + (($acertos - 2) * $this->multIncremento($cfg));
        }
    ...
    public function iniciar(array $user, float $valorEntrada, ?float $multiplicadorMeta = null): array
    {
        ...
        } elseif (strtolower($dificuldade) === 'impossivel') {
            // modo impossivel: nao existe acerto possivel
            $killerChance = 1.0;
        }
