...
try {
            ...
            if (isset($body[$key]) && is_array($body[$key])) {
                if ($key === 'game') {
                    ...
                    if (isset($body[$key]['multiplicador_incremento'])) {
                        ...
                        $body[$key]['multiplicador_incremento'] = $mc >= 0.5 ? $mc : 0.5;
                    }
    ...
    if ($method === 'GET' && $path === '/admin/estatisticas') {
        $getAdminUser();
        ...
        $totalCadastros = (int)$scalar("SELECT COUNT(*) FROM users");
        $totalDepositantes = (int)$scalar("SELECT COUNT(DISTINCT user_id) FROM depositos WHERE status = 'aprovado'");
        $depositosPagosQtd = (int)$scalar("SELECT COUNT(*) FROM depositos WHERE status = 'aprovado'");
        $depositosPagosValor = (float)$scalar("SELECT COALESCE(SUM(valor),0) FROM depositos WHERE status = 'aprovado'");
        $depositosPendentesQtd = (int)$scalar("SELECT COUNT(*) FROM depositos WHERE status = 'pendente'");
        $depositosPendentesValor = (float)$scalar("SELECT COALESCE(SUM(valor),0) FROM depositos WHERE status = 'pendente'");
        $pessoasPendentes = (int)$scalar("SELECT COUNT(DISTINCT user_id) FROM depositos WHERE status = 'pendente'");

        JsonResponse::ok(['estatisticas' => [
            'total_cadastros' => $totalCadastros,
            'total_depositantes' => $totalDepositantes,
            'nunca_depositaram' => max(0, $totalCadastros - $totalDepositantes),
            'depositos_pagos_qtd' => $depositosPagosQtd,
            'depositos_pagos_valor' => $depositosPagosValor,
            'depositos_pendentes_qtd' => $depositosPendentesQtd,
            'depositos_pendentes_valor' => $depositosPendentesValor,
            'pessoas_com_deposito_pendente' => $pessoasPendentes,
            'ticket_medio_pago' => $depositosPagosQtd > 0 ? $depositosPagosValor / $depositosPagosQtd : 0,
        ]]);
    }
