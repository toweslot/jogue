...
final class AmploPayProvider implements PaymentProviderInterface
{
    ...
    private function request(string $method, string $path, ?array $body = null): array
    {
        ...
        // Relay proprio (Cloudflare Worker gratuito). Quando AMPLOPAY_BASE_URL
        // aponta para o Worker, este header autentica a chamada no Worker.
        $relaySecret = trim((string)($this->cfg['relay_secret'] ?? ''));
        if ($relaySecret !== '') {
        ...
        for ($i = 0; $i < $attempts; $i++) {
            ...
            curl_setopt_array($ch, [
                ...
                CURLOPT_USERAGENT => 'TowerSlot-Server/1.0 (+https://towerslot.site)',
            ]);

            // Saida por proxy (necessario quando o IP da hospedagem esta
            // bloqueado no WAF/CDN da AmploPay). Ex.: http://user:pass@ip:porta
            $proxy = trim((string)($this->cfg['proxy'] ?? ''));
            if ($proxy !== '') {
                curl_setopt($ch, CURLOPT_PROXY, $proxy);
                curl_setopt($ch, CURLOPT_HTTPPROXYTUNNEL, true);
                $proxyAuth = trim((string)($this->cfg['proxy_auth'] ?? ''));
                if ($proxyAuth !== '') {
        ...
        if ($this->isHtmlBody((string)$resp)) {
            throw new RuntimeException(
                ...
                . '. O IP de saida da hospedagem esta bloqueado no WAF/CDN da AmploPay (nao e a lista de IPs Autorizados do painel, que so vale para transferencias). Confira se AMPLOPAY_BASE_URL no .env aponta para o relay (veja LEIA-ME-CLOUDFLARE.md); se estiver apontando direto para app.amplopay.com o WAF volta a bloquear. Detalhes em api/storage/amplopay.log.'
            );
        }

        if ($code === 401 && str_contains((string)$resp, 'forbidden')) {
            throw new RuntimeException(
                'O relay recusou a chamada (401). O valor de AMPLOPAY_RELAY_SECRET no .env esta diferente do configurado no relay.'
            );
        }
    ...
    public function createDeposit(array $payload): array
    {
        ...
        if ($email === '') {
            $email = 'user' . substr(bin2hex(random_bytes(4)), 0, 8) . '@towerslot.site';
        }
